...
//            wbg_start: None,
    ...
    fn with_read_hook(&mut self, fidx: usize) {
        self.read_hook = Some(fidx)
    }

    fn with_write_hook(&mut self, fidx: usize) {
...
pub fn add_hooks(
    ...
    for (i, table) in &mut module.tables.iter_mut().enumerate() {
...
//                "__wbindgen_start" => hook_builder.with_wbg_start(fidx),
    ...
    module.functions.push(write_hook);
